...
public class CryptographyGUI extends JFrame {
    ...
    private JPanel createEncryptionPanel() {
        ...
        // Create output panel with equal height
    ...
    private JPanel createPasteButtonPanel(JTextArea targetArea) {
        JPanel buttonPanel = new JPanel(new FlowLayout(FlowLayout.RIGHT, 5, 0));
        buttonPanel.setBackground(BACKGROUND_COLOR);
        
        JButton pasteButton = new JButton("Paste from Clipboard");
        pasteButton.setFont(REGULAR_FONT);
        pasteButton.setBackground(ACCENT_COLOR);
        pasteButton.setForeground(Color.WHITE);
        pasteButton.addActionListener(e -> {
            try {
                String clipboardText = (String) Toolkit.getDefaultToolkit()
                    .getSystemClipboard().getData(DataFlavor.stringFlavor);
                targetArea.setText(clipboardText);
            } catch (Exception ex) {
        ...
        buttonPanel.add(createStyledButton(pasteButton));
        return buttonPanel;
    }
    ...
    // Helper method to find ComboBox in container
    private JComboBox<?> findComboBox(Container container, String defaultValue) {
        for (Component comp : container.getComponents()) {
            if (comp instanceof JComboBox) {
                return (JComboBox<?>) comp;
            } else if (comp instanceof Container) {
                JComboBox<?> result = findComboBox((Container) comp, defaultValue);
                if (result != null) {
                    return result;
                }
            }
